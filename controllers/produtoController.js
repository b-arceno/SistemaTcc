...
exports.listar = (req, res) => {
  db.query('SELECT id, nome, descricao, preco, categoria_id, imagem FROM Produto', (err, results) => {
...
exports.buscarPorId = (req, res) => {
  ...
  db.query('SELECT id, nome, descricao, preco, categoria_id, imagem FROM Produto WHERE id = ?', [id], (err, result) => {
    if (err) return res.status(500).json({ error: 'Erro ao buscar produto', detail: err.message });
    if (!result[0]) return res.status(404).json({ error: 'Produto não encontrado' });
...
exports.deletar = (req, res) => {
  ...
  db.query('DELETE FROM Produto WHERE id = ?', [id], (err) => {
    if (err) return res.status(500).json({ error: 'Erro ao deletar produto', detail: err.message });
